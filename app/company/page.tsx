          ...
          <Reveal delay={150}>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { stat: '1,750+', label: 'Engineers' },
                { stat: '8', label: 'Stocking locations' },
                { stat: '24/7', label: '× 365 availability' },
                { stat: 'L1–L5', label: 'Engineer levels' },
                { stat: '20+', label: 'Languages spoken' },
              ].map((item) => (
                <div
                  key={item.label}
                  className="rounded-xl border border-navy-100 bg-navy-50 p-6 text-center"
                >
                  <p className="text-2xl font-bold text-navy-950">{item.stat}</p>
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <Reveal delay={150}>
            <Card className="space-y-6">
              {expertise.map((e) => (
                <SkillBar key={e.skill} skill={e.skill} level={e.level} />
              ))}
            </Card>
          </Reveal>
        </div>
